...
class RegisterMod extends Component {
  constructor(props) {
    super(props);
    this.state = {
      email: null,
  ...
  handleSubmit = (event) => {
    ...
    if(this.state.serv === '' || this.state.mech === '' || this.state.email === null || this.state.creditCard === null){
      this.setState({pass:'すべてのフィールドに正しく入力してください' })
                ...
                <Form.Group as={Row}>
                  ...
                  <Col sm={5}>
                    ...
                    <Form.Control as="select" value ={this.state.serv} onChange = {this.handleServ} defaultValue="">
                      ...
                      <option value ="oilChange">$400 - 着色ウィンドウ</option>
                      <option value ="flatTire">$800 - 排気の変更</option>
                      <option value ="paintTouchUp">$1200 - フルカーラップ</option>
                      <option value ="engineRepair">$3000 - フルエンジンチューン</option>
                    </Form.Control>
                  </Form.Group>
                  </Col>
                </Form.Group>
